...
\begin{frame}{Contextualização do problema}
    ...
    Alocar médicos de ultrassonografia em uma \textbf{rede de clínicas}, atendendo a demanda de exames conforme a disponibilidade de cada profissional (\(\approx\)15\,min/exame \cite{conselho_regional_de_medicina_do_estado_do_parana_2022}).

    \begin{figure}
        \centering
        \includegraphics[width=0.62\linewidth]{img/exemplo-schedule-medico.png}
...
\begin{frame}{Funções objetivo: um problema bi-objetivo}
    \begin{columns}[T]
...
\begin{frame}{Detecção de trocas: variável de estado \(\gamma_{iud}\)}
    \begin{columns}[T]
