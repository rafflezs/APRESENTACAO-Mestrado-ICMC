...
\begin{frame}{Contextualização do problema}
    ...
    % \vspace{-1cm}
    \begin{center}
        Médicos \(\rightarrow\) Clínicas (Conjunto) \(\rightarrow\) Demanda \\
    ...
    \begin{small}
        \begin{columns}[T]
            ...
            \begin{column}{0.6\textwidth}
                ...
                \begin{tabular}{ll}
                    \(x_{iudt}\) & Alocação de \(i\) em \(u\), dia \(d\), turno \(t\) \\
                    \(w_{iud}\)  & Alocação de \(i\) em \(u\) no dia \(d\) \\
                    \(c_{udt}\)  & Exames não atendidos \\
                    \(z_{id}\)   & Contabilização de trocas \\
                    \(\gamma_{iud}\) & Mudança de unidade entre dias \\
                \end{tabular}
