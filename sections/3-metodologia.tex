...
\begin{frame}{O método $\varepsilon$-restrito}
    \begin{columns}[c]
...
\begin{frame}{Meta-heurística: NSGA-II}
    ...
    \begin{columns}[T]
        \begin{column}{0.33\textwidth}
            \begin{itemize}\item Ordenação não-dominada.\end{itemize}
        \end{column}
        \begin{column}{0.33\textwidth}
            \begin{itemize}\item Distância de \textit{crowding}.\end{itemize}
        \end{column}
        \begin{column}{0.33\textwidth}
            \begin{itemize}\item Elitismo (pais $\cup$ filhos).\end{itemize}
        \end{column}
    \end{columns}
    \begin{figure}
        \centering
...
\begin{frame}{Operadores customizados: perpetuando a espécie}

    {\footnotesize Amostragem inicial \textbf{com consciência de restrições} $+$ perservação da coerência temporal:}
%     \begin{columns}[T]
...
%     \end{columns}
    \begin{figure}
        \centering
        \includegraphics[width=0.75\linewidth]{img/02_crossover.png}
        \caption{\footnotesize Cruzamento por blocos diários.}
    \end{figure}

    \begin{figure}
        \centering
        \includegraphics[width=0.75\linewidth]{img/03_mutacao.png}
        \caption{\footnotesize Mutação por realocação de \textit{slot}.}
    \end{figure}
\end{frame}

%%% Frame 3.5 — Encoding Compacto vs Estendido
\begin{frame}{Encoding: Estendido \emph{vs.} Compacto}
    \begin{figure}
        \centering
